...
int sensorInitOptions(struct EpsonOptions options) {
  ...
  int atti_ctrl_hi = (options.gyro_delta_out & 0x01) << 1 |
                     ((options.atti_out | options.qtn_out) & 0x01) << 2 |
...
void registerDump(void) {
  ...
  registerRead16(0x00, 0x52, debug);
  registerRead16(0x00, 0x54, debug);
  printf("\r\n");
  registerRead16(0x00, 0x56, debug);
  registerRead16(0x00, 0x58, debug);
  registerRead16(0x00, 0x5A, debug);
  printf("\r\n");
  registerRead16(0x00, 0x5C, debug);
  registerRead16(0x00, 0x5E, debug);
#endif  // G365PDF1
